...
- 문제 요약: 접근성 설정(확대 모드)에서 키패드 UI가 화면 밖으로 잘려 보이지 않는 문제
- UI/UX 개선 제안:
...
`;
    ...
    const response = await openai.chat.completions.create({
      ...
      messages: [
        {
          role: "system",
          content: "당신은 UI/UX/GUI/Flow 전문 디자이너입니다. 사용자 리뷰를 분석하여 구체적이고 실행 가능한 인터페이스 개선 제안을 생성합니다. 

요구사항:
1. **구체적인 UI 컴포넌트**: 버튼 위치/크기/색상, 아이콘 종류, 레이아웃 배치를 명시
2. **사용자 플로우**: 단계별 화면 전환과 터치포인트를 구체화
3. **인터랙션 디자인**: 제스처, 애니메이션, 피드백 방식을 상세히 기술
4. **GUI 요소**: 팝업, 다이얼로그, 상태 표시를 구체적으로 설계

예시: '키패드가 잘려서 안 보임' → '화면 배율 150%일 때 키패드 크기 80% 자동 조정', 'ScrollView 컨테이너로 세로 스크롤 지원', '드래그 가능한 플로팅 키패드 토글 버튼'

반드시 실제 리뷰 내용을 기반으로 구체적인 UI/UX 솔루션을 제안하고, 유효한 JSON 형태로 답변해주세요."
        },
        {
          role: "user",
          content: prompt
