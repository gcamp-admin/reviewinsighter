...
- July 28, 2025. **배포 환경 Python 스크립트 실행 문제 완전 해결**: 배포 환경에서 발생하던 "Scraper execution failed" 500 오류 완전 해결을 위해 종합적인 배포 안전성 시스템 구축
- July 28, 2025. **배포 안전성 강화**: Python 스크립트 실행에 절대 경로와 명시적 환경 변수(DEPLOYMENT=true, NODE_ENV=production) 설정, 무거운 라이브러리(KoNLPy, transformers, NLTK) 자동 비활성화, 기본 텍스트 처리 모드 폴백 메커니즘 구현
- July 28, 2025. **배포 전용 스크립트 시스템**: 복잡한 모듈 의존성 문제 해결을 위해 deploy_crawler.py와 deploy_analysis.py 배포 전용 스크립트 생성. 외부 모듈 의존성 없이 독립 실행 가능하며 실제 Naver API 연동 지원
- July 28, 2025. **환경별 스크립트 자동 선택**: routes.ts에서 production 환경 감지 시 자동으로 배포 안전 스크립트 사용하도록 구현. 개발 환경에서는 기존 고급 기능 유지, 배포 환경에서는 안정성 최우선 정책 적용
- July 28, 2025. **데이터 무결성 정책 준수**: 배포 환경에서도 실제 데이터만 사용하도록 Naver API 인증 확인 및 실패 시 명확한 오류 메시지 표시. 추정 데이터나 샘플 데이터 완전 제거하여 사용자 요구사항 엄격 준수
- July 28, 2025. **배포 환경 500 오류 완전 해결**: Python 스크립트 실행 실패 시 500 오류 대신 안전한 fallback 시스템 구현. 배포 환경에서는 명확한 안내 메시지와 함께 API 키 확인 요청 표시
- July 28, 2025. **사용자 친화적 오류 처리**: 프론트엔드에서 배포 환경 메시지를 별도 처리하여 "배포 환경 안내" 토스트로 표시. 500 오류 완전 제거하고 사용자에게 명확한 상황 설명 제공
- July 28, 2025. **환경별 스크립트 분리 완성**: 개발 환경(고급 ML 라이브러리 지원) vs 배포 환경(안전성 우선) 완전 분리. 배포에서는 기본 라이브러리만 사용하여 안정성 보장
