        ...
        <div className="space-y-4">
          ...
          <div className="flex flex-col items-center space-y-3 pt-2">
            <Button 
              ...
              className="w-full bg-black hover:bg-black disabled:bg-black px-6 py-3 text-lg font-semibold hover:shadow-lg hover:scale-105 transition-all duration-300 disabled:hover:scale-100"
              size="lg"
            >
              {collectReviewsMutation.isPending ? (
                <>
                  <Loader2 className="w-5 h-5 mr-3 animate-spin" style={{ color: '#cdff55' }} />
                  <span className="animate-pulse" style={{ color: '#cdff55' }}>수집 중... {collectionProgress}%</span>
                </>
              ) : (
                <>
                  <Search 
                    className={`w-5 h-5 mr-3 group-hover:scale-110 transform transition-transform duration-300 ${collectReviewsMutation.isPending || !localFilters.service || localFilters.source.length === 0 || !localFilters.dateFrom || !localFilters.dateTo || hasDateRangeError ? 'opacity-50' : ''}`} 
                    style={{ color: '#cdff55' }} 
                  />
                  <span 
                    className={collectReviewsMutation.isPending || !localFilters.service || localFilters.source.length === 0 || !localFilters.dateFrom || !localFilters.dateTo || hasDateRangeError ? 'opacity-50' : ''} 
                    style={{ color: '#cdff55' }}
                  >
                    리뷰 수집
                  </span>
